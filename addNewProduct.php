...
<body>
    ...
    <div class="col-md-3" style="margin-left: 90px;"><!-- col-md-3 -->
        <div class="panel panel-default sidebar-menu"><!--panel panel-default sidebar-menu  -->
            ...
            <div class="panel-body"><!--panel-body  -->
                <div class="profile-links">
                    ...
                    <a href="cusDeliveredOrders.php"><i class="fa fa-users">View Delivered Customer Orders</i></a>  
    ...
    <div class="form-container">
        ...
        <form class="myForm" action="../Tennis_Joint/Product_Addition.php"  method="POST">

            <input placeholder="Product Name" type="text" name="prodName" id="prodName">
            <input placeholder="Colour" type="text" name="colour"  id="colour">
            ...
            <select name=selectCategory id="selectCategory">
                ...
                <?php
                    $getOptions = mysqli_query($conn, "select * from categories") or die("Unable to read data");
                    $resultConfirm = mysqli_num_rows($getOptions);

                    if ($resultConfirm > 0) 
                    {
                        while ($myCat=mysqli_fetch_array($getOptions))
                        {
                            echo '<option value="'.$myCat['idcategories'].'">'.$myCat['name'].'</option>';
                        }
                    }
                ?>
            
            </select>
            <select name=selectTargetMarket id="selectTargetMarket">
                <option value="0"> Target Market</option>
                <?php
                    $getMarket = mysqli_query($conn, "select * from targetMarket") or die("Unable to read data");
                    $resultConfirm2 = mysqli_num_rows($getMarket);

                    if ($resultConfirm2 > 0) 
                    {
                        while ($myMark=mysqli_fetch_array($getMarket))
                        {
                            echo '<option value="'.$myMark['idtargetMarket'].'">'.$myMark['Name'].'</option>';
                        }
                    }
                ?>
            </select>
            ...
            <select name=selectType id="selectType">
                ...
                <?php
                    $getType = mysqli_query($conn, "select * from types") or die("Unable to read data");
                    $resultConfirm3 = mysqli_num_rows($getType);

                    if ($resultConfirm3 > 0) 
                    {
                        while ($myTypes=mysqli_fetch_array($getType))
                        {
                            echo '<option value="'.$myTypes['idTypes'].'">'.$myTypes['Name'].'</option>';
                        }
                    }
                ?>
            </select>
            ...
            <select name=selectBrand id="selectBrand">
                ...
                <?php
                    $getBrand = mysqli_query($conn, "select * from brand") or die("Unable to read data");
                    $resultConfirm4 = mysqli_num_rows($getBrand);

                    if ($resultConfirm4 > 0) 
                    {
                        while ($myBrand=mysqli_fetch_array($getBrand))
                        {
                            echo '<option value="'.$myBrand['idBrand'].'">'.$myBrand['name'].'</option>';
                        }
                    }
                ?>
            </select>
            <br>
            <input type="file" class="prodImage" name="prodImage"id="prodImage" >
            <br>
            <input type="file" class="prodImage" name="prodImage2"id="prodImage" >
            <br>
            <input type="file" class="prodImage" name="prodImage3"id="prodImage" >
            <br>
